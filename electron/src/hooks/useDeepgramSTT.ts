...
function micErrorMessage(err: unknown): string {
  if (err instanceof DOMException) {
    if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
      return "Microphone access was denied. Allow the microphone in System Settings > Privacy.";
    }
...
/**
 * Server-relayed live STT (WebM → socket → OpenAI Whisper / Deepgram on server).
 */
export function useDeepgramSTT(
  ...
  /** Count of in-flight `arrayBuffer` → `stt_audio` sends so `stt_stop` runs after the last chunk. */
  const pendingAudioSendsRef = useRef(0);
  /** When true, emit `stt_stop` only after recorder `stop` and `pendingAudioSendsRef === 0`. */
